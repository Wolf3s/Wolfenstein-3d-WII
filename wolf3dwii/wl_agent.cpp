...
void ControlMovement (objtype *ob)
{
    ...
    if( controlz < 0 )     // variable strafe to the left
	{
		...
		Thrust (angle,controlz*MOVESCALE);
	}

	if( controlz > 0 )     // variable strafe to the right
	{
		angle = ob->angle + ANGLES/4;
		...
		Thrust (angle,-controlz*MOVESCALE);     // move to right
	}
