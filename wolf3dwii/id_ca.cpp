...
//mrp begin I don't think we need this...	#include <sys/uio.h>
	#include <unistd.h>
#endif
...
void CAL_GetGrChunkLength (int chunk)
{
	...
	chunkexplen = wiiLongSwap(chunkexplen);
...
//
	...
	{
		pictable[i].height = wiiShortSwap(pictable[i].height);
		pictable[i].width = wiiShortSwap(pictable[i].width);
	}
...
//
	...
	if (chunk == STARTFONT | chunk == STARTFONT + 1)
		wiiProcessFont(chunk);
}
