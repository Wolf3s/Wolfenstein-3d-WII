...
int buttonscan[NUMBUTTONS] = { sc_Control, sc_Alt, sc_LShift, sc_Space, sc_1, sc_2, sc_3, sc_4, sc_Return, sc_None, sc_None, sc_None, sc_Z, sc_X };
...
/* MrP - Heavily modified this function to NOT accept input for y dimension (forward/back) and got rid of mouse centering
         so you don't have to keep moving the wiimote to turn.  This function is used for turning your player using the mouse cursor position
		 (0,0) (topleft) from SDL
*/
...
void PollJoystickMove (void)
{
    ...
    IN_GetJoyDelta (&joyx, &joyy, &joyz);

    int delta = RUNMOVE * tics; // the fastest pace possible


	if(joyx > delta)  // delta is the fastest speed you can go, which is run
		joyx = delta;
	if(joyx < -delta)
		joyx = -delta;
	if(joyy > delta)
    ...
    if (joyx > 16 || buttonstate[bt_turnright])   // 16 is pretty precise
        controlx += joyx *0.6;
    else if (joyx < -16  || buttonstate[bt_turnleft])
        controlx += joyx *0.6;
